...
以下描述的是 ADR 0005 之前的链路，代码保留但不参与日常发布，也不接受新的候选材料。当前手工发布见下一节。
...
`manual-gateway.mjs` 只接收四个动词：`manual-baseline`、`manual-upload ID SHA256`、`manual-release ID BASELINE`、`manual-status ID`。上传经标准输入，只接受 `application.tar.gz` 与 `receipt.json` 两个固定名称文件，限制压缩/展开体积、核对 bundle 摘要后原子落进 root 持有的受限 incoming 目录；不接受路径参数、不执行 shell、不读取秘密配置，响应只含白名单内的公开事实与已知机器码，服务器原始错误文本不出边界。同一 ID 重复上传摘要必须一致，已完成/失败/未知的 ID 只读历史、绝不重新派发。

服务器发布前交叉核对三处身份：受限 `runtime.json`、当前目录 `release.json` 与活跃进程 readiness 自报提交，任一不符即拒绝；`slots` 配置存在时全部手工写动词直接拒绝。发布在持锁后校验收据（模式、提交、运行时、判据清单、预期结构版本、结构变更确认），复用带全新临时库的本机预检，再进入维护、停服、独占数据锁生成发布前配套快照、换 `current`、启动并按判据验收；开放流量前持久写入可能开放标记。判据含进程 active、自报版本、回环深度健康的 schema/完整性/附件、登录页及其引用资产，以及本站点 JSON access log 中除明确维护响应外的新增 5xx。资产可读不等同浏览器执行渲染，也不含业务写入探针。
...
每份有效快照包含数据归档、原运行产物、原 Node 二进制、单独受限的秘密配置、版本/数据时间清单和 SHA-256。备份目录权限 0700，材料 0600；配置不进入普通应用构建包或日志。SQLite 完整性与外键、附件引用、大小和指纹均校验，并重新解包归档核验文件摘要。半成品目录不接受恢复。产物归档与正在服务的代码必须**逐文件比摘要一致**（`build`、`dist`、`node_modules`、`package.json`、`package-lock.json`、`release.json`）；只核对 `release.json` 的标签不足以证明归档就是运行中的代码，`package-lock.json` 只描述预期的依赖树、不证明已安装字节，所以就地改过服务目录、热修过依赖或未换产物时以 `SERVED_CODE_MISMATCH` 拒绝，不能留下恢复点。这些代码根目录本身是符号链接或套接字等特殊文件时先以 `UNSAFE_SERVED_CODE_TYPE` 拒绝，归档内含链接或设备时在读取其中清单前以 `UNSAFE_ARCHIVE_LINK` 拒绝。
