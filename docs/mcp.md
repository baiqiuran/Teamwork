...
`get_context` 向任一有效连接返回当前成员、授权、服务器时间和北京时间日期。工具只显示当前获准范围；实际调用仍复核权限。提交包含新任务或状态意图的草稿时，还必须有任务能力。只可读写自己的私人草稿、管理自己的分享和连接；团队进展只读提交版本。查询与写入范围由被授权成员所属团队决定：客户端传入的其他成员、项目、任务、日报或团队标识不能替换主体，也不能扩到别的团队，跨团队对象一律按未找到处理。
...
成员可以在「AI 连接 → 通过 Node 连接」创建命名 Key，选择五类能力（默认查询和本人草稿）。Key 仅在本次创建后可取用，默认不渲染正文；成员主动复制或显示后，自行用本地编辑器保存为仓库外密钥文件，不交给模型录入。Key 持续有效至撤销；连接列表和操作记录不返回 Key，数据库只存摘要。通过已有撤销入口关闭指定 Key，其他 Key 和 OAuth 连接继续有效。

生成配置只含 `DAILY_FLOW_URL` 和 `DAILY_FLOW_API_KEY_FILE` 绝对路径，不含 Key 正文。本地包启动时读取文件，通过 stdio 接收 AI 客户端调用，转为携带 Bearer Key 的远程 `/mcp` 请求；仍支持从可信启动环境传入 `DAILY_FLOW_API_KEY`，但两种来源同时配置时拒绝启动，文件无效时不回退。工具及业务能力以服务器为准，写入沿用 `operationId` 去重和版本冲突规则。创建 Key 需要网页 Cookie 登录和同源请求；Key 不具备签发其他 Key 的能力。

已有 Key 无需强制重签，迁移时由成员自行移除明文配置，改为文件引用并重启、执行只读查询确认；若已进入聊天或日志，应创建新 Key、验证后撤销旧 Key。引用方式防止意外暴露，不阻止拥有相应本机读取权限的模型工具取得密钥；不要通过输出文件、环境变量或请求头来验证真实认证。

构建、npx 本地 tarball 使用方式及 Windows 配置见 [Node 包接入说明](../packages/mcp-node/README.md)。该包尚未发布到公共 npm，不要直接执行未核实的同名公共包。此次新增方式不替代下面的 OAuth 接入。
...
网页「AI 连接 → 连接 Codex」只显示一段可编辑、可复制的 PowerShell 命令，先添加远程服务器，再发起本人授权。命令中的 MCP 地址默认是当前站点的 `/mcp`；若 Codex 在另一台电脑上，请直接将命令中的 URL 改为该电脑可访问的同一日序服务 HTTPS 地址。命令结构如下：

```powershell
codex mcp add daily_flow --url "https://team.example.com/mcp" --oauth-client-id daily-flow-codex
if ($LASTEXITCODE -eq 0) { codex mcp login daily_flow --scopes progress:read,drafts:write }
```

已有同名连接时，`mcp add` 不会覆盖原地址。也可以手动将下面的配置加入自己的 Codex `config.toml`：
...
1. 无账号访客可在站点 `/setup` 创建新团队，只填写团队名称、姓名、邮箱和密码，不需要引导密钥或本机隧道；已有账号使用 `/login`，加入已有团队仍须邀请。创建入口持续开放，`/api/setup/status` 的布尔值 `needsSetup` 只表示站点是否尚无团队，不是创建权限。公网访问先完成下述 HTTPS 与可信源地址配置，不必先在本机创建团队。
...
域名或公网 IP、证书文件及进程托管配置由真实服务器环境补齐。不要将 4310 暴露公网。代理覆盖来源头，不能沿用客户端伪造的 `X-Forwarded-For`。应用校验 Host 和存在的 Origin；网页写操作仍必须同源，受保护的业务写操作还须带 Cookie，公开创建不需要已有会话，MCP 使用 Bearer。不要在代理中封锁 `/setup` 或 `/api/setup`；创建仍受密码规则和频率限制。HTTPS 代理访问的会话 Cookie 带 Secure。发现、OAuth 和 MCP 路由不会回退为 SPA HTML。
...
本版使用 SQLite 有序迁移，支持版本到 7。增加客户端、授权、凭证摘要、回执及审计；显式重建任务事件表，保留旧 ID/成员/任务/日报/时间，将旧事件标记为 diary/web；独立事件为 direct/mcp 且无日报 ID。版本 7 增加 Key 摘要和连接名称、凭证类型，旧连接默认 OAuth，既有凭证及业务数据保留。迁移全部成功后才监听，失败回滚并释放资源；超出支持版本的数据库会被拒绝。
