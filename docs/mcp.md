...
网页「AI 连接 → 连接 Codex」会生成一段可复制的 PowerShell 命令，先添加远程服务器，再发起本人授权。远程 MCP 地址默认是当前站点的 `/mcp`；若 Codex 在另一台电脑上，请填入该电脑可访问的同一日序服务 HTTPS 地址。命令结构如下：
...
已有同名连接时，`mcp add` 不会覆盖原地址。也可以在页面展开「手动配置服务器」，将下面的配置加入自己的 Codex `config.toml`：
