...
成员可以在「AI 连接 → 通过 Node 连接」创建命名 Key，选择五类能力（默认查询和本人草稿）。Key 只在创建时显示，持续有效至撤销；连接列表和操作记录不返回 Key，数据库只存摘要。通过已有撤销入口关闭指定 Key，其他 Key 和 OAuth 连接继续有效。

本地包以 `DAILY_FLOW_URL` 和 `DAILY_FLOW_API_KEY` 配置，通过 stdio 接收 AI 客户端调用，转为携带 Bearer Key 的远程 `/mcp` 请求。工具及业务能力以服务器为准，写入沿用 `operationId` 去重和版本冲突规则。创建 Key 需要网页 Cookie 登录和同源请求；Key 不具备签发其他 Key 的能力。
