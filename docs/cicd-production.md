...
**以下是历史接管记录，不是当前发布操作顺序。** 自动发布与双槽切换链路已不再使用；受管备份与保留链路仍在使用，不可整套删除。本文记录的 2026-09-22 blue 槽状态已经过期；2026-09-23 已完成单实例接管与首次手工发布。当前状态、发布与人工恢复见 [手工发布手册](manual-release.md)。
