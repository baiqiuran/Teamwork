...
**历史实施记录，不是当前发布入口。** 按 ADR 0005，自动发布与双槽切换链路已不再使用，文件保留；受管备份与保留链路仍在使用，不能删除 `ops/`。2026-09-22 只读实测生产仍运行 blue 槽，单实例手工发布尚未在生产切换。手工发布与人工恢复见 [手工发布手册](manual-release.md)。

以下保留旧固定提交构建、迁移说明门禁和 CI 验证方法，只可按需手动用于开发验收；手工发布不调用这些旧门禁。
...
首次生产跨度补充：文件搬迁需精确绑定旧 blob、目标路径与新 blob，删除或对应错误停止；候选验证再次独立检查计划，不能通过手工改计划跳过。组合根的哈希 `.update(...)` 不属于 SQLite UPDATE；实际 UPDATE/DELETE/DROP/REPLACE 仍拦截。历史 DDD 文件变更已逐项复核，最终部署仍需真实基线到最终 main 产物的升级与恢复证明。
