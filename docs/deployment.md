...
**历史建站记录，不是当前发布操作顺序。** 2026-09-23 已切为 `daily-flow.service` 单实例并完成首发；自动发布与切槽链路已不再使用，备份与保留链路仍在使用。当前发布与人工恢复见 [手工发布手册](manual-release.md)。下文的服务器构建、旧路径、启用服务和旧备份命令仅供理解历史，不可直接在现有生产执行。
