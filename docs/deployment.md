...
**历史建站记录，不是当前发布操作顺序。** 2026-09-22 只读实测仍由 blue 槽服务，旧 `daily-flow.service` 未启用。自动发布与切槽链路已不再使用，备份与保留链路仍在使用；新的单实例流程尚待生产切换。当前发布设计与人工恢复见 [手工发布手册](manual-release.md)。下文的服务器构建、旧路径、启用服务和旧备份命令仅供理解历史，不可直接在现有生产执行。
