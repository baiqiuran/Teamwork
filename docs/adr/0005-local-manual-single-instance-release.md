...
维护者授权后，生产从 `daily-flow@blue.service` 切为 `daily-flow.service`，旧 reconcile 入口禁用，备份与保留 timer 保留；受限基线入口返回 `mode=single`。随后固定产物 `manual-a9084f06-c7bf-4709-8924-cb2eca6702de` 发布提交 `402863b909615ed0b3d74a44473dd40f11e581f3`，记录的维护耗时 18,815 毫秒，数据库 schema 9、完整性通过。实际操作与尚未验证的原公开链接见[手工发布手册](../manual-release.md)。
