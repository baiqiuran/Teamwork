...
当前代码提供同一站点多团队的网页与成员授权 MCP 服务：创建团队持续开放，一个账号只属于一个团队，团队内部数据相互隔离；已验证 Codex 接入。公开链接支持免登录读取。2026-09-23 已在单台 Ubuntu ECS 上完成单实例接管和首次手工发布，生产运行提交 `402863b909615ed0b3d74a44473dd40f11e581f3`。自动发布已停用，现行发布和恢复步骤见[手工发布与人工恢复](docs/manual-release.md)。
...
既有业务已扩展为同一站点多团队：成员、日报、项目任务、附件、邀请、公开分享与 MCP 操作均按团队隔离，创建团队入口持续开放，一个账号只属于一个团队。以上隔离与旧库升级已在本地通过 HTTP、MCP 与浏览器验收，2026-09-23 首次手工发布后生产数据库结构为 v9。尚未提供邮件发送、密码找回或审批。
...
- [手工发布与人工恢复](docs/manual-release.md)：当前发布方案、首次生产切换记录、日常发布与故障处理。
