...
项目按 DDD 模块化单体组织：后端分为领域、应用、基础设施和 HTTP 接口，前端按成员、日报、项目任务、分享组织。参阅 [架构与事务说明](docs/architecture.md)、[业务术语](CONTEXT.md) 和 [架构决策](docs/adr/0001-modular-monolith.md)。
...
`npm test` 运行架构依赖检查、类型检查与构建、HTTP 业务测试、Edge 浏览器流程。业务测试使用真实服务和 SQLite，时钟只在测试创建应用时注入。覆盖持久化、成员隔离、跨日、提交幂等、项目重新归集、任务快照/冲突/事务回滚、全部记录筛选、三类模块分享、20 MB 附件及归档撤权。
