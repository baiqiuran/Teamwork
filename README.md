...
当前提供同一站点多团队的网页与成员授权 MCP 服务：创建团队持续开放，一个账号只属于一个团队，团队内部数据相互隔离；已验证 Codex 接入。公开链接支持免登录读取。2026-09-18 已在单台 Ubuntu ECS 上完成公网 IP HTTPS 部署与真实网页登录验收，2026-09-20 已完成双槽单活接管与本地备份恢复验收，现行运维入口见 [CI/CD 生产手册](docs/cicd-production.md)。多团队改造目前只在本地完成验收，尚未发布到该服务器。
...
既有业务已扩展为同一站点多团队：成员、日报、项目任务、附件、邀请、公开分享与 MCP 操作均按团队隔离，创建团队入口持续开放，一个账号只属于一个团队。以上隔离与旧库升级已在本地通过 HTTP、MCP 与浏览器验收；生产仍运行升级前部署的版本，需要先按发布与一致快照流程升级。尚未提供邮件发送、密码找回、审批或自动化公网部署。
...
- [CI/CD 生产接入与运维](docs/cicd-production.md)：本地备份、GitHub 配置、首次切换及真实验收清单。

- [阿里云 ECS 部署手册](docs/deployment.md)：公网 IP HTTPS、创建与登录、备份恢复及更新。
